...
var importObject: any = importObject || {},
  moduleInstance: any = moduleInstance || {},
  wasmModule: any = wasmModule || {},
  getImportObject: any = getImportObject || (() => { });
...
/////////////////////////////////////////
// START ASSEMBLYSCRIPT WORKER RUNTIME //
/////////////////////////////////////////
...
function preInstantiate(imports) {
  ...
  env.abort = env.abort || function abort(msg, file, line, colm) {
    const memory = (extendedExports as any).memory || env.memory; // prefer exported, otherwise try imported
    throw Error(`abort: ${getString(memory, msg)} at ${getString(memory, file)}:${line}:${colm}`);
  };
  env.trace = env.trace || function trace(msg, n, ...args) {
    const memory = (extendedExports as any).memory || env.memory;
  ...
  function getView(alignLog2, signed, float) {
    ...
    if (float) {
      switch (alignLog2) {
        ...
        case 3: return new Float32Array(buffer);
      }
    } else {
      switch (alignLog2) {
        ...
        //case 3: return new (signed ? Int64Array : Uint64Array)(buffer);
      }
    }
...
async function wascStreaming(source, imports = {}) {
  if (!WebAssembly.instantiateStreaming) {
    throw Error("WebAssembly.instantiateStreaming not supported!");
  }
...
///////////////////////////////////////
// END ASSEMBLYSCRIPT WORKER RUNTIME //
///////////////////////////////////////
  ...
  if (action === ACTIONS.COMPILE_MODULE) {
    Promise.resolve()
      .then(() => {
        ...
        return wascStreaming(fetch(payload), importObject);
      })
      .then(({ module, instance }) => {
        // remembr module
        wasmModule = module;
        moduleInstance = instance;
        // resolve available methods
        onSuccess({
