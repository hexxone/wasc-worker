...
export default function worker(e) {
  ...
  const BIGINT = false; // typeof Uint64Array !== "undefined";
  const THIS = Symbol();
  ...
  function postInstantiate(extendedExports, instance) {
    ...
    if (BIGINT) {
      /*
      [Uint64Array, Int64Array].forEach(ctor => {
      ...
      .then(() => {
        ...
        if (typeof payload === 'string') {
          ...
          if (instantiateStreaming === undefined) {
            console.error("assemblyscript-loader not found! Fallback to WebAssembly...");
            res = res.then(response => response.arrayBuffer());
          }
          else return instantiateStreaming(res, importObject);
        } else {
